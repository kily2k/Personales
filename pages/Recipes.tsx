...
import { Product, RecipeItem, UnitType, Ingredient, Family } from '../types';
...
export const Recipes: React.FC = () => {
  const { products, ingredients, families, addProduct, updateProduct } = useBakery();
  ...
  // Effect to scroll to top when form opens (new or edit)
  useEffect(() => {
    if (isCreating) {
      // Try multiple ways to scroll to top to ensure it works in all environments (iframe, etc.)
      topRef.current?.scrollIntoView({ behavior: 'auto' });
      
      // Target the main scrollable container directly if possible
  ...
  const handleEditClick = (product: Product) => {
    ...
    setProductFamilyId(product.familyId || '');
  ...
  const handleSaveProduct = async () => {
    ...
    if(!isIntermediate && (productPrice === '' || isNaN(priceAsNumber) || priceAsNumber < 0)) {
    ...
    try {
        ...
        const finalPrice = productPrice === '' ? 0 : priceAsNumber;

        const prodData: Product = {
            id: editingProduct ? editingProduct.id : Date.now().toString(),
            name: productName,
            price: finalPrice,
            familyId: productFamilyId || undefined, // Context will clean this up
            recipe: validItems,
        ...
        <div className="bg-white p-6 rounded-xl border border-indigo-100 shadow-lg space-y-6 animate-in fade-in slide-in-from-top-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
                ...
                {!isIntermediate && (
                  <div>
                    <label className="label text-sm font-medium text-slate-700 mb-1">Familia / Categoría</label>
                    <select
...
}> = ({ product, ingredients, products, families, onEdit }) => {
            ...
            <div className="p-4 flex justify-between items-center hover:bg-opacity-80">
                ...
                >
                    ...
                    <p className="text-slate-500 text-sm flex items-center gap-2">
                        {product.isIntermediate ? 'Receta Base / Preparado' : (
                          <>
                            {familyName && <span className="bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase">{familyName}</span>}
                            {product.recipe.length} componentes
                          </>
                        )}
                    </p>
                </div>
