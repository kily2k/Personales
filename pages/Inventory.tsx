...
import { Plus, Search, Edit2, X, RefreshCw, TrendingUp, Trash2 } from 'lucide-react';
...
export const Inventory: React.FC = () => {
  ...
  const [editMinStockValue, setEditMinStockValue] = useState<string>(''); // For Min Stock
  ...
  const handleEditClick = (ing: Ingredient) => {
    ...
    const minVal = fromBaseUnit(ing.minStock, ing.unit);
    setEditStockValue(val.toString());
    setEditMinStockValue(minVal.toString());
  ...
  const handleSaveEdit = () => {
    ...
    const minVal = parseFloat(editMinStockValue);
    ...
    const newMinStockBase = toBaseUnit(minVal, editingItem.unit);
    
    // Check if price is provided for WAC
    const priceVal = newPurchasePrice ? parseFloat(newPurchasePrice) : undefined;

    updateIngredientStock(editingItem.id, newStockBase, priceVal, newMinStockBase);
                  ...
                  <tr key={ing.id} className={`hover:bg-slate-50 ${isLow ? 'bg-red-50' : ''}`}>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {isLow && (
                          <span className="bg-red-100 text-red-600 text-[10px] px-1.5 py-0.5 rounded font-black uppercase tracking-tighter animate-pulse">
                            Bajo
                          </span>
                        )}
                      </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input 
                      type="number"
                      step="any"
                      className="w-full p-2 border border-slate-300 rounded-lg font-bold text-center focus:ring-2 focus:ring-indigo-500 outline-none"
