...
import { ProductionRequirement, Order } from '../types';
import { formatStock } from '../utils/conversions';
import { Calendar, ClipboardList } from 'lucide-react';

export const Production: React.FC = () => {
  const { orders, products, ingredients, getIngredientUsage } = useBakery();
  ...
  const requirements = useMemo(() => {
    ...
    // 2. Aggregate all items from all orders for that day
    ...
    // 3. Use the recursive calculator from context
    const map = getIngredientUsage(allItems);
  ...
  }, [orders, products, ingredients, selectedDate, getIngredientUsage]);
