...
import { Ingredient, Product, Order, OrderStatus, UnitType, Customer, MonthClosure, Family } from '../types';
...
interface BakeryContextType {
  ...
  updateIngredientStock: (id: string, newAmount: number, newUnitCost?: number, newMinStock?: number) => void;
  ...
  getIngredientUsage: (items: Order['items']) => Map<string, number>;
}
...
export const BakeryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubFamilies = onSnapshot(collection(db, 'families'), (snap) => {
      setFamilies(snap.docs.map(d => ({ ...d.data(), id: d.id } as Family)));
    }, (err) => {
  ...
  const getRecursiveIngredientUsage = (
      ...
      if (type === 'INGREDIENT' || !type) { // Default to Ingredient if undefined
          // Safety check: only add if ingredient exists in our local state
          const exists = ingredients.some(i => i.id === itemId);
          if (!exists) {
            console.warn(`Ingredient with ID ${itemId} not found in inventory. Skipping stock deduction.`);
            return;
          }
  ...
  const updateIngredientStock = async (id: string, newAmount: number, newUnitCost?: number, newMinStock?: number) => {
    const ing = ingredients.find(i => i.id === id);
    if (!ing) return;

    let updates: Partial<Ingredient> = { currentStock: newAmount };
    if (newMinStock !== undefined) updates.minStock = newMinStock;
  ...
  const addProduct = async (prod: Product) => {
    ...
    const cleanData = Object.fromEntries(Object.entries(rest).filter(([_, v]) => v !== undefined));
  ...
  const addFamily = async (family: Family) => {
    ...
    const cleanData = Object.fromEntries(Object.entries(rest).filter(([_, v]) => v !== undefined));
    await addDoc(collection(db, 'families'), cleanData);
  };

  const updateFamily = async (updated: Family) => {
    const { id, ...rest } = updated;
    const cleanData = Object.fromEntries(Object.entries(rest).filter(([_, v]) => v !== undefined));
    await updateDoc(doc(db, 'families', id), cleanData);
  };
  ...
  const updateOrder = async (updatedOrder: Order) => {
    try {
      ...
      oldUsage.forEach((amount, ingId) => {
          ...
          if (ingredients.some(i => i.id === ingId)) {
            batch.update(ingRef, { currentStock: increment(amount) });
          }
      });
  ...
  const performMonthClose = async () => {
    ...
    // Calculate Total Cost for these orders
    const allItems = openPaidOrders.flatMap(o => o.items);
    const usage = getIngredientUsage(allItems);
    let totalCost = 0;
    usage.forEach((amount, ingId) => {
      const ing = ingredients.find(i => i.id === ingId);
      if (ing) {
        // amount is in base units, convert to display unit for cost calculation
        const displayAmount = fromBaseUnit(amount, ing.unit);
        totalCost += displayAmount * ing.costPerUnit;
      }
    });
